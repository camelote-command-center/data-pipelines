-- ============================================================================
-- 2026-09-13 — Unblock warehouse -> foncier (foncier-geneve) distribution
-- ============================================================================
-- Foncier received NOTHING since 2026-08-10 (data frozen at 2026-08-06): every target of
-- run_foncier_sync (transactions, sad, listings daily; plots monthly) was REFUSED by
-- gold_ch.sync_full_refresh's fail-closed guard — 109 refusals — while the cron job reported
-- success in ~0 s. Before the guard, foncier was fed by the destructive legacy TRUNCATE path.
--
-- Fix: Branch A, the non-destructive path the guard itself names as correct.
--   staging TRUNCATE -> FDW load into staging -> INSERT ... ON CONFLICT (pk) DO UPDATE
-- It never truncates or deletes live foncier rows (consistent with the no-deletions rule).
-- Verified before building: all 4 targets carry a unique key on the registry pk_column; no
-- generated/identity columns; every registry column exists in the source view and on foncier;
-- nothing on foncier writes to these tables (content_editorial_snapshot only reads them).
--
-- Steps
--  (1) foncier: ref._staging_{transactions,sad,listings} UNLOGGED + ref._staging_plots LOGGED
--      (LIKE live INCLUDING DEFAULTS) — same persistence as lamap_db's staging tables.
--  (2) re-LLM: foncier_foreign._staging_* foreign tables. NOT via IMPORT FOREIGN SCHEMA:
--      foncier has PostGIS in schema "extensions", re-LLM in "public", so the imported type
--      extensions.geometry does not exist on re-LLM. DDL generated from foncier's staging
--      columns with the geometry type mapped, matching the working live foreign tables.
--  (3) re-LLM: gold_ch.sync_full_refresh
--      - 4 foncier tuples added to c_allowlist;
--      - Branch A key/column lookup falls back to gold_ch.foncier_sync_registry, scoped to
--        p_target_db = 'foncier' (run_foncier_sync passes a NULL column list and Branch A read
--        only gold_ch.sync_registry, so allowlisting alone would have failed "registry row
--        missing pk_column or column_list"). Unchanged for every other target.
--      Previous definition (md5 53db193d2f7fc38252a00fb2fa644aa4) backed up in backup.fn_defs_20260913.
--
-- ROLLBACK: restore the backed-up sync_full_refresh definition (foncier returns to REFUSED and
-- keeps whatever it received). The staging tables and foreign tables are inert on their own.
-- ============================================================================

-- (1) run on foncier-geneve:
-- CREATE UNLOGGED TABLE IF NOT EXISTS ref._staging_transactions (LIKE ref.transactions INCLUDING DEFAULTS);
-- CREATE UNLOGGED TABLE IF NOT EXISTS ref._staging_sad          (LIKE ref.sad          INCLUDING DEFAULTS);
-- CREATE UNLOGGED TABLE IF NOT EXISTS ref._staging_listings     (LIKE ref.listings     INCLUDING DEFAULTS);
-- CREATE          TABLE IF NOT EXISTS ref._staging_plots        (LIKE ref.plots        INCLUDING DEFAULTS);

-- (2) run on re-LLM:
CREATE FOREIGN TABLE IF NOT EXISTS foncier_foreign._staging_listings (
  listing_id text,
  source text,
  source_id text,
  title text,
  ...
  price_unit text,
  rent_net numeric,
  rent_charges numeric,
  rent_gross numeric,
  currency text,
  rooms numeric,
  surface_living_m2 numeric,
  surface_usable_m2 numeric,
  surface_land_m2 numeric,
  floor text,
  npa text,
  city text,
  street text,
  ...
  dedup_group_hash text,
  price_per_m2 numeric,
  has_price boolean,
  has_coordinates boolean,
  room_category text,
  price_category text,
  fts_listing tsvector,
  geometry geometry,
  ...
  admin3_code text,
  admin3_name text,
  admin3_canonical_id text
) SERVER foncier_server OPTIONS (schema_name 'ref', table_name '_staging_listings');
CREATE FOREIGN TABLE IF NOT EXISTS foncier_foreign._staging_plots (
  egrid text,
  no_commune_no_parcelle text,
  geometry geometry(MultiPolygon,4326)
) SERVER foncier_server OPTIONS (schema_name 'ref', table_name '_staging_plots');
CREATE FOREIGN TABLE IF NOT EXISTS foncier_foreign._staging_sad (
  sad_id text,
  permit_number integer,
  permit_number_complement text,
  permit_type character varying,
  permit_type_full text,
  status text,
  description text,
  commune text,
  communes_all text[],
  ...
  proprietaire text,
  proprietaire_address text,
  requerant_principal text,
  requerant_principal_address text,
  requerant_secondaire text,
  date_depot date,
  ...
  volume_sia_m3 numeric,
  sbp_total_m2 numeric,
  ...
  type_operation text,
  operation text,
  code_instance_directrice character varying,
  detenteur character varying,
  id_autorisation character varying,
  is_remarque_da_diffusable boolean,
  is_remarque_diffusable boolean,
  libelle_instance_directrice character varying,
  numero_bordereau character varying,
  origine_dossier character varying,
  remarque_da character varying,
  remarque_dossier character varying,
  requerant_secondaire_adresse character varying,
  statut_da character varying,
  commune_canonical_bfs integer,
  country_code character(2),
  admin1_code text,
  admin1_name text,
  admin2_code text,
  ...
  admin3_canonical_id text,
  no_commune_no_parcelle text
) SERVER foncier_server OPTIONS (schema_name 'ref', table_name '_staging_sad');
CREATE FOREIGN TABLE IF NOT EXISTS foncier_foreign._staging_transactions (
  transaction_id character varying,
  affaire_number character varying,
  source_id text,
  transaction_date date,
  publication_date date,
  transaction_type text,
  transaction_type_list text[],
  price numeric,
  commune text,
  commune_number text,
  canton_code text,
  plot_references text[],
  egrids text[],
  primary_egrid text,
  plot_surface_m2 numeric,
  price_per_m2 numeric,
  ppe_indicator boolean,
  ppe_pourmille_raw text,
  buildings_raw jsonb,
  transaction_text text,
  buyer_count integer,
  seller_count integer,
  buyers_display text,
  sellers_display text,
  buyer_names text[],
  seller_names text[],
  property_type text,
  surface_m2 numeric,
  ...
  admin3_name text,
  admin3_canonical_id text,
  parties_buyers jsonb,
  parties_sellers jsonb
) SERVER foncier_server OPTIONS (schema_name 'ref', table_name '_staging_transactions');
