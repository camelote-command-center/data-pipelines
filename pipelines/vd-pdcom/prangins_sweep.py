...
BATCH_SHA='460e5f25fa5465837b39d54d076b35712cc7197f31e8df36cd102109a3b770c9'
KEYS={'aerodrome', 'port', 'cemetery', 'green_urban_room', 'beach', 'sports', 'public_space', 'vineyards', 'historic_centres', 'rdu_parkway', 'public_parks', 'public_utility_proposed', 'existing_equipment', 'intermediate_densification', 'jardins_proposal', 'allocation_unknown', 'green_equipment_band', 'mixed_extensions'}
...
def validate(b):
    ...
    if len(b['features'])!=18 or {f['key'] for f in b['features']}!=KEYS:raise ValueError('prangins_scope')
    ...
    if pairs!=420:raise ValueError('prangins_pair_count')
    return b
def load_artifacts():
    raw=(Path(__file__).parent/'reports/prangins-sweep/batch.json').read_bytes()
