...
stack=[];ledger=[];groups=collections.defaultdict(list);wanted={d.get('layer') for d in page.get_drawings(extended=True)}-{'BATI','LEGENDE','village','recensement archi','densification'}
for i,d in enumerate(page.get_drawings(extended=True)):
 ...
 try:
  g=compound(d['items'],d.get('even_odd',False))
  for z in stack:
   if z['type']=='clip':
