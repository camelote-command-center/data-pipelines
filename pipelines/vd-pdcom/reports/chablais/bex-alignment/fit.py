...
P=np.array(json.loads((root/'source-points.json').read_text()));Q=np.array([r[1:] for r in json.loads((root/'reference-points.json').read_text())]);P0=P.mean(0);Q0=np.array([2566500.,1122200.]);P-=P0;Q-=Q0
...
for ang in range(-20,21,5):
 for scale in [.65,.75,.85,.95,1.05,1.15]:
...
for _,x in sorted(best,key=lambda z:-z[0])[:8]:
 for k in range(20):
  ...
  x=least_squares(lambda z:(trans(train[mask],z)-Q[idx[mask]]).ravel(),x,loss='soft_l1',f_scale=2,bounds=([-.8,.45,-1500,-1500],[.8,1.5,1500,1500])).x
