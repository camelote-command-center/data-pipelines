...
P=np.array(json.loads((root/'source-points.json').read_text()));Q=np.array([r[1:] for r in json.loads((root.parent/'bex-alignment/reference-points.json').read_text())]);P0=P.mean(0);Q0=np.array([2566500.,1122200.]);P-=P0;Q-=Q0
train=P[np.arange(len(P))%5!=0];test=P[np.arange(len(P))%5==0]
cell=4;size=800;origin=-1600
def grid(points):
...
for ang in range(-20,21,5):
 for scale in [.75,.9,1.05,1.2,1.35,1.5]:
...
# Approximate visual seed from same courtyard building, not a surveyed control.
old=json.loads((root.parent/'bex-alignment/alignment.json').read_text())
a,sc,tx,ty=old['parameters'];rr=np.array([[np.cos(a),-np.sin(a)],[np.sin(a),np.cos(a)]])
q=(np.array([373.,-674.])-old['source_origin_y_flipped'])@rr.T*sc+[tx,ty]+np.array(old['reference_origin_lv95'])-Q0
for sc in [.9,.95,1.,1.05,1.1]:
 for a in [-.08,0,.08]:
  shift=q-trans(np.array([524.,-548.])-P0,[a,sc,0,0]);best.append((1e6,[a,sc,*shift]))
solutions=[]
for _,x in sorted(best,key=lambda z:-z[0])[:23]:
 for k in range(20):
  dist,idx=tree.query(trans(train,x));mask=dist<max(5,25-k)
  if mask.sum()<20:break
  x=least_squares(lambda z:(trans(train[mask],z)-Q[idx[mask]]).ravel(),x,loss='soft_l1',f_scale=2,bounds=([-.8,.45,-1500,-1500],[.8,1.8,1500,1500])).x
