...
# Geographic research geometry, never inserted as validated sectors.
from shapely.ops import transform
src=json.loads((root.parent/'vector-outline/extraction.json').read_text());features=[]
def geo(x,y,z=None):
 q=trans(np.column_stack([x,-np.asarray(y)]));return q[:,0],q[:,1]
for key in ['inner','outer','band']:
 g=transform(geo,shape(src['priority_densification'][key]));features.append({'type':'Feature','properties':{'boundary_variant':key,'status':'research_only'},'geometry':mapping(g)})
(root/'priority-band-lv95.geojson').write_text(json.dumps({'type':'FeatureCollection','coordinate_system':'EPSG:2056','features':features}))
