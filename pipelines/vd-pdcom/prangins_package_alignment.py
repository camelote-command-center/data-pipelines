...
def validate_alignment(b):
    ...
    if len(rows)!=56 or len({r['official_attributes']['OBJECTID'] for r in rows})!=56 or len({r['official_attributes']['EGID'] for r in rows})!=56:raise ValueError('inset_control_identity')
    for r in rows:
    ...
    if (len(train),len(hold))!=(34,22):raise ValueError('inset_control_partition')
    ...
    if abs(max(err)-b['alignment']['heldout_max_m'])>1e-6 or abs(np.sqrt(np.mean(np.square(err)))-b['alignment']['holdout_rmse_m'])>1e-6:raise ValueError('inset_heldout_evidence')
    hull=MultiPoint([r['source_point'] for r in train]).convex_hull
    for f in b['features']:
        for p in f['source_paths']:
            g=shape(p['geometry_base_pdf']);fraction=(g.intersection(hull).area/g.area if g.area else g.intersection(hull).length/g.length)
