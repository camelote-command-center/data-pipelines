...
def persist(conn,document_id,sha):
    ...
    with conn,conn.cursor() as c:
        ...
        sector_ids=[str(uuid.uuid5(uuid.NAMESPACE_URL,str(document_id)+'#page99-extdrawing='+str(f['properties']['extended_drawing_index'])+'#calibration=2026-09-14')) for f in features]
        for f in features:
