...
BATCH_SHA='4093af45aeaa9b420c820367b03ff0266dc9ffa52c014cabb0cff5fd64d1db79'
...
def validate(b):
    ...
    for f in b['features']:
        ...
        for p in f['paths']:
            g=shape(p['geometry_lv95']);orig=shape(p['geometry_pdf'])
            ...
            if f['feature_kind']=='source_native_point':
                from prangins_symbol_anchor import anchor
                if not orig.equals_exact(anchor(p,a['allowed_extraction']),1e-7):raise ValueError('native_anchor')
        g=combine(f['paths'],f['feature_kind'])
        if not shape(f['geometry_lv95']).equals_exact(g,1e-7):raise ValueError('native_collection')
    return b
def load_artifacts():
    raw=(Path(__file__).parent/'reports/prangins-native-final/batch.json').read_bytes()
...
def persist(conn,document_id,sha):
    ...
    with conn,conn.cursor() as c:
        ...
        if not stored or stored[0]!=SHA or not stored[1] or stored[1]<1:raise ValueError('native_stored_source')
        for f in b['features']:
            ...
            evidence=dict(feature_kind=f['feature_kind'],source_sha256=SHA,source_path_ids=[p['path_id'] for p in f['paths']],source_category=f['label'],legend_label_id=sem['label_id'],source_ledger_index=sem['source_ledger_index'],page_number=1,semantic_contract=sem,source_transform=b['affine'],alignment=b['alignment'],source_precision='unknown',source_precision_m=None,review_status='review_required',publication_status='internal_review_only',grouping='Source-native category collection; paths and symbol anchors are not parcels, zoning areas or measured legal boundaries',parcel_association='none: non-area source feature',geometry_processing='Original boundary centreline and explicitly reviewed source-glyph anchor rule; no buffers, polygon invention or cadastral intersection',batch_sha256=BATCH_SHA,artifact='pipelines/vd-pdcom/reports/prangins-native-final')
            ...
            c.execute('''INSERT INTO bronze_ch.vd_pdcom_sectors(id,document_id,page_number,label,geom,alignment_rmse_m,validation_evidence,review_status)
                VALUES(%s,%s,1,%s,ST_Transform(ST_SetSRID(ST_GeomFromGeoJSON(%s),2056),4326),%s,%s,'review_required') ON CONFLICT DO NOTHING''',(sid,DOC,label,g,b['alignment']['holdout_rmse_m'],Json(evidence)))
