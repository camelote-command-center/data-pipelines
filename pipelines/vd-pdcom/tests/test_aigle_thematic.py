...
class ThematicTests(unittest.TestCase):
 ...
 def test_scope(self):
  self.assertEqual(len(self.b['features']),2)
  self.assertEqual(sum(len(f['expected_pairs']) for f in self.b['features']),622)
  self.assertEqual(len({p.sector_id(k) for k in p.KEYS}),2)
 ...
 def test_legacy_replay_scoped(self):
  ...
  conn=Conn();features=[{'properties':{'extended_drawing_index':n,'label':'old'},'geometry':{}} for n in range(11)]
  import json
  with patch.object(Path,'read_text',side_effect=[json.dumps({'holdout_rmse_m':1,'holdout_median_m':1}),json.dumps({'features':features}),json.dumps({})]):old.persist(conn,p.DOC,p.SHA)
  inserts=[(q,a) for q,a in conn.queries if 'WITH p AS MATERIALIZED' in q]
  self.assertEqual(len(inserts),1);q,args=inserts[0]
  self.assertIn('WHERE id=ANY(%s::uuid[])',q);self.assertEqual(len(args[0]),11)
