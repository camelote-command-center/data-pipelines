...
class ThematicTests(unittest.TestCase):
 ...
 def test_scope(self):
  ...
  self.assertEqual(sum(len(f['expected_pairs']) for f in self.b['features']),66)
 ...
 def test_nonadditive(self):
  ...
  a,b=self.b['features'];self.assertGreater(shape(a['geometry_lv95']).intersection(shape(b['geometry_lv95'])).area,1500)
