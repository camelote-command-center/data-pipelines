...
class ThematicTests(unittest.TestCase):
 ...
 def test_scope(self):
  ...
  self.assertEqual(sum(len(f['expected_pairs']) for f in self.b['features']),361)
 ...
 def test_duplicate_prior(self):self.reject(lambda b:b['features'][0]['properties'][0].update(layer='densification'))
 ...
 def test_legacy_replay_scoped(self):
  ...
  conn=Conn();features=[{'properties':{'drawing_index':n,'label':'old'},'geometry':{}} for n in range(15)]
  import json
  with patch.object(Path,'read_text',side_effect=[json.dumps({'holdout_rmse_m':1,'holdout_median_m':1}),json.dumps({'features':features})]):old.persist(conn,p.DOC,p.SHA)
  inserts=[(q,a) for q,a in conn.queries if 'WITH parcels AS MATERIALIZED' in q]
  self.assertEqual(len(inserts),1);q,args=inserts[0]
  self.assertIn('WHERE id=ANY(%s::uuid[])',q);self.assertEqual(len(args[0]),15)
