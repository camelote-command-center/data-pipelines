...
class ThematicTests(unittest.TestCase):
 ...
 def test_scope(self):
  self.assertEqual(len(self.b['features']),18)
  self.assertEqual(sum(len(f['expected_pairs']) for f in self.b['features']),420)
  self.assertEqual(len({p.sector_id(k) for k in p.KEYS}),18)
