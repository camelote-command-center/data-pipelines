...
import prangins_package_areas as p
class ThematicTests(unittest.TestCase):
 ...
 def test_scope(self):
  ...
  self.assertEqual(sum(len(f['expected_pairs']) for f in self.b['features']),11)
 ...
 def test_union(self):self.reject(lambda b:b['features'][0].update(geometry_lv95={'type':'Polygon','coordinates':[[[0,0],[1,0],[0,1],[0,0]]]}))
 ...
 def test_composite_once(self):
  self.assertEqual(len(self.b['features']),1)
  self.assertEqual(self.b['features'][0]['source_ledger_indices'],[])
  self.assertEqual(len(self.b['features'][0]['source_paths']),1)
