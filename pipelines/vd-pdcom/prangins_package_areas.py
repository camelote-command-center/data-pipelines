...
BATCH_SHA='c1755568a1cf3522f5e14c14835765bccc00df364f36e2f987372a5a5b4994b0'
KEYS={'ppa_eglantine_peupliers_situation'}
...
def validate(b):
    ...
    for f in b['features']:
        ...
        for path in f['source_paths']:
            ...
            orig=shape(path['geometry_base_pdf'])
            if not orig.equals_exact(translate(shape(path['geometry_pdf']),-f['translation'][0],-f['translation'][1]),1e-7):raise ValueError('package_source_frame')
            expected=affine_transform(orig,m)
    ...
    if pairs!=11:raise ValueError('prangins_pair_count')
    return b
def load_artifacts():
    raw=(Path(__file__).parent/'reports/prangins-package-maps/areas.json').read_bytes()
...
def persist(conn,document_id,sha):
    ...
    with conn.cursor() as c:
        ...
        if not d or d[0]!=SHA or not d[1] or d[1]<75:raise ValueError('prangins_stored_source')
