...
BATCH_SHA='21276190754adaa98e604045f63227bed43829558652b9776983a5c4aac591b8'
KEYS={'heritage_grades_1_3','heritage_classified_grade_1'}
...
def validate(b):
    ...
    for f in b['features']:
        ...
        if any(pr['category']!=f['category'] or pr['page']!=f['page'] or pr['source_sha256']!=SHA or pr['layer']!='recensement archi' or pr['fill']!=({'heritage_grades_1_3':[.966,.923,.077],'heritage_classified_grade_1':[.779,.127,.151]}[f['key']]) for pr in f['properties']):raise ValueError('prangins_semantics')
        eg=set()
    ...
    return b
def load_artifacts():
    raw=(Path(__file__).parent/'reports/prangins-heritage/batch.json').read_bytes()
