...
DROP TRIGGER IF EXISTS ge_rdppf_synthese_content_hash_trg ON ref.ge_rdppf_synthese;
CREATE TRIGGER ge_rdppf_synthese_content_hash_trg
  BEFORE INSERT OR UPDATE ON ref.ge_rdppf_synthese
  FOR EACH ROW EXECUTE FUNCTION ref.ge_rdppf_synthese_content_hash();

-- Backfill every existing row through the trigger.
UPDATE ref.ge_rdppf_synthese SET content_hash = NULL;

CREATE INDEX IF NOT EXISTS ge_rdppf_synthese_content_hash_idx ON ref.ge_rdppf_synthese (content_hash);
