...
INCREMENTAL STRATEGY:
    ...
    - Page through results, stop when all records on a page are older than
      the cutoff date (a fixed rolling RESCAN_LOOKBACK_DAYS window anchored to now())
...
# Fixed rolling re-scan window (days). Every run re-fetches dossiers whose
...
# date_statut = 06.06.2070 in bronze). We clamp anything beyond now()+1y to NULL
# at write time so dirty source data never propagates downstream. NOTE: this
# excludes forward-looking fields like date_fin_valid_instance_directrice
# (validity END dates) which CAN legitimately be in the future.
PAST_DATE_FIELDS = ("date_statut", "date_depot", "date_acceptation")
...
def transform_record(raw: dict) -> dict:
    ...
    # Clamp implausible future dates to NULL (defends against SITG sentinels
    # like 06.06.2070). Only historical event-date fields — see PAST_DATE_FIELDS.
    horizon = datetime.now() + timedelta(days=DATE_HORIZON_DAYS)
    for field in PAST_DATE_FIELDS:
        parsed = parse_sad_date(record.get(field))
        if parsed and parsed > horizon:
            print(f"    Clamped future {field}={record[field]} → NULL "
                  f"(dossier {record.get('type')} {record.get('numero')})")
            record[field] = None
...
def main():
    ...
    # ROOT FIX (2026-05-28): use a FIXED rolling re-scan window anchored to now(),
    # NOT last_acquired_at. The previous logic set cutoff = last_acquired_at — the
    # job's wall-clock run time, which the workflow PATCHes to now() on EVERY
    # success (even when zero rows are captured). Combined with comparing
    # date-only SAD fields (midnight) against that mid-day timestamp, the window
    # ratcheted forward to ~today and silently excluded every new dossier; the
    # feed froze at date_depot 2026-05-13 while runs kept reporting "success".
    # A fixed lookback is self-correcting (can never ratchet past the data),
    # idempotent (UPSERT on conflict key), and immune to dirty future dates in
    # date_statut (observed 2070-06-06 in bronze) — so it does NOT read a
    # possibly-corrupt MAX(date) watermark. The window must exceed the typical
    # gap between runs plus any backlog; 45d catches up the current freeze and
    # provides ample margin for missed/late runs going forward.
    # (meta is still read below for the record_count log line.)
    meta = get_dataset_meta(camelote_url, camelote_key, DATASET_CODE)
    if meta:
        print(f"\n  Previous record count: {meta.get('record_count', 'unknown')}")
    # Normalize to midnight so the >= comparison against date-only SAD fields is
    # inclusive of the entire cutoff day.
    cutoff_date = (datetime.now() - timedelta(days=RESCAN_LOOKBACK_DAYS)).replace(
        hour=0, minute=0, second=0, microsecond=0
    )
    print(f"  Cutoff: {cutoff_date.strftime('%Y-%m-%d')} "
          f"(fixed rolling {RESCAN_LOOKBACK_DAYS}-day re-scan window)")
