#!/usr/bin/env python3
"""
SITG Geography — Import Pipeline (config-driven, pure ArcGIS)

Fetches 6 SITG datasets via ArcGIS REST API and upserts into lamap_db:
  1. CAD_COMMUNE                    — Municipal boundaries        (polygon)
  2. CAD_PISCINE                    — Swimming pools              (polygon)
  3. CAD_VERANDA                    — Verandas                    (polygon)
  4. GOL_SITES_POLLUES              — Polluted sites              (polygon)
  5. GEO_ADRESSE_AGGLO              — Agglomeration addresses     (point)
  6. AGR_SURFACE_AGRICOLE_RECENSEE  — Agricultural surfaces       (polygon)
...
DATASETS = [
    ...
    },
    {
        ...
        "conflict_column": "objectid",
        # Table already uses shape__area / shape__length (double underscore) — no rename
    },
    {
        ...
        "conflict_column": "objectid",
        # ArcGIS returns 'id' field → rename to 'adresse_id' to match table (avoid PK clash)
        "field_renames": {
            "id": "adresse_id",
        },
    },
    {
        ...
        "conflict_column": "objectid",
        # ArcGIS returns 'id' field → rename to 'provider_id' to match table (avoid PK clash)
        "field_renames": {
            "id": "provider_id",
            "shape__length": "shape_len",
            "shape__area": "shape_area",
